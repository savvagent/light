...
- [x] Add `OfflineReason` enum (derives `Debug, Clone, PartialEq, Eq`; variants `NothingConfigured`,
      `NamedProviderMissingKey { selector, key }`, `BaseUrlRejected { var }`).
- [x] Add `pub offline: Option<OfflineReason>` and `pub warnings: Vec<String>` to `BuiltProvider`.
- [x] Introduce internal `RemoteSelection { choice, offline, warnings }` and `BaseUrlRejection {
...
- [x] Rewire `build_provider_from_env` to populate `offline` + `warnings`.
- [x] Update existing selection tests (they now assert against `RemoteSelection`/`Result`) and add
...
- [x] Run `cargo test -p light-factory-providers` (expect green).
- [x] Run `cargo fmt --all` and commit `providers: carry offline selection reason and warnings`.
...
- [x] Add `fn is_offline(&self) -> bool { false }` to `Provider` (default body) with a doc comment.
- [x] Override `is_offline() -> true` in `LocalProvider` (`crates/providers/src/local.rs`).
- [x] Add a unit test in `local.rs` asserting `LocalProvider::new().is_offline()` is `true`, and a
      test in `crates/providers/src/scripted.rs`'s test module asserting `ScriptedProvider` is
      `false`.
- [x] Run `cargo test -p light-factory-engine-core -p light-factory-providers` (expect green).
- [x] Run `cargo fmt --all` and commit `engine-core: add Provider::is_offline seam`.
...
- [x] Add the guard at the top of `Session::run_turn` (`turn.rs:49`), before `propose_plan`:
      ...
      provider).
- [x] Add a test in `crates/engine/tests/turn.rs` that spawns a `Session` with `LocalProvider`,
...
- [x] Run `cargo test -p light-factory-engine` (expect green).
- [x] Run `cargo fmt --all` and commit `engine: report no_provider_configured for offline turns`.
...
- [x] Extend `ProviderInfo` with `offline: Option<OfflineReason>` and `warnings: Vec<String>`;
      populate from `BuiltProvider`.
- [x] Add pure `offline_notice(locale, &OfflineReason) -> String` in `provider.rs` mapping the three
      variants to `provider.offline.*` keys.
- [x] In `app.rs::enter_engine`, capture `info` (not `_info`) and, **after**
      `self.engine_log.clear()`, push every `info.warnings` line then the `offline_notice` when
      offline.
- [x] Add EN + ES entries (`provider.offline.nothing`, `provider.offline.missing_key`,
      `provider.offline.base_url`, `error.no_provider_configured`), ES mirroring EN.
- [x] Add a unit test for `offline_notice` (three reasons) in `provider.rs`; the i18n EN/ES parity
      test already enforces mirroring.
- [x] Run `cargo test -p light-factory-tui` (expect green).
- [x] Run `cargo fmt --all` and commit `tui: surface offline reason and selection warnings`.

### Task 5: Workspace verification

**Files:** none (verification only).

- [x] Run `cargo test --workspace` (persistence PG test skips without `DATABASE_URL`).
- [x] Run `cargo clippy --workspace --all-targets -D warnings`.
- [x] Run `cargo fmt --all --check`.
- [x] Out-of-band: no `Dockerfile`/`fly.toml`/`web/`/`migrations/`/`.github/` touched — state
      "none" in the PR.
