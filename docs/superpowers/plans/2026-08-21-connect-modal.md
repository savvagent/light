...
- [ ] Re-export from `lib.rs`: `pub mod models;` and re-export **only** the validating wrappers `pub use models::{list_models, list_ollama_models};`. The `_at` variants stay `pub(crate)` so no external caller can hit them with an unvalidated base URL (the trust boundary stays enforced at the only public entry points).
