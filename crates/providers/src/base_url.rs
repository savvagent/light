...
mod tests {
    ...
    #[test]
    fn the_completion_client_carries_no_deadline() {
        ...
        for base in ["https://api.openai.com", "http://127.0.0.1:8080"] {
            ...
            // one is configured, so its absence is the client-level assertion.
            let rendered = format!("{client:?}").to_lowercase();
            assert!(
                !rendered.contains("timeout"),
                "the completion client must carry no deadline: {rendered}"
            );
